...
def bag_tree(data):
    ...
    lines = [l.split(" bags contain ") for l in lines]


    # rules = [line.strip(" .").split(" bags contain ") for line in data.strip().split("\n")]
    ...
    for rule in lines:
        ...
        for child in children:
            ...
            ch = (child_data[0], " ".join(child_data[1:-1]))
            if ch[0] == "no":
                continue
...
def bfs(tree, root):
    ...
    while stack:
        ...
        for child in tree[curr]:
            num = int(child[0])
            count += num
            stack.extend([child[1]]*num)
    return count

def main(data):
    tree = bag_tree(data)
    return bfs(tree, "shiny gold")

data = """dim silver bags contain 3 posh fuchsia bags.
