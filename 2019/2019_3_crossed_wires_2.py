...
def foo(data):
    grid = Counter()
    wires = [[(x[0], int(x[1:])) for x in line.split(",")] for line in data.splitlines()]

    path1 = map_path(wires[0])
    path2 = map_path(wires[1])
    
    shortest = math.inf
    for point in path1.keys():
        if point in path2.keys():
            shortest = min(path1[point] + path2[point], shortest)
    return shortest
...
def map_path(wire):
    move = {
        "U": (lambda point: (point[0], point[1]+1)),
        "D": (lambda point: (point[0], point[1]-1)),
        "R": (lambda point: (point[0]+1, point[1])),
        "L": (lambda point: (point[0]-1, point[1]))
    }
